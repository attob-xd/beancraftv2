...
public class BlockPatternBuilder {
   ...
   /**
    * Vanilla allocates this with {@code Array.newInstance(Predicate.class, depth, height,
    * width)} - the multi-dimensional overload, which TeaVM's {@code java.lang.reflect.Array}
    * does not have (it has only the single-dimension form). Reflection is not doing anything
    * here that the language cannot: the reflective call exists purely because a generic array
    * cannot be written as {@code new Predicate<BlockInWorld>[..][..][..]}. Allocating the raw
    * type and keeping vanilla's unchecked cast produces the identical array, with no
    * reflection involved.
    *
    * <p>This is on the path for every block pattern in the game - nether portals, snow and
    * iron golems - so it is reached the first time a player builds one.
    */
   @SuppressWarnings({ "unchecked", "rawtypes" })
   private Predicate<BlockInWorld>[][][] createPattern() {
      this.ensureAllCharactersMatched();
      Predicate[][][] var1 = new Predicate[this.pattern.size()][this.height][this.width];
