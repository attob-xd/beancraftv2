   ...
   protected final int runUpdates(int var1) {
      if (this.firstQueuedLevel >= this.levelCount) {
         ...
         while (this.firstQueuedLevel < this.levelCount && var1 > 0) {
            var1--;
            // LevelRenderer calls runUpdates(Integer.MAX_VALUE, ...) once per frame, so this
            // loop's only real bound is the queue draining. If light never converges - a
            // position whose level keeps being recomputed and re-enqueued - it runs two billion
            // times, which on the single thread a browser tab has is a frozen page rather than
            // a slow one: no frames, no packets, no keep-alive, and nothing in the log to say
            // where it stopped. This makes that case say so.
            if (++var20 % 1000000 == 0) {
               LOGGER.warn("Light engine has run {} updates in one pass and the queue still holds {}",
                  var20, this.computedLevels.size());
            }
