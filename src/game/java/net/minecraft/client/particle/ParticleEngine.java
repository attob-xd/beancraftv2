...
public class ParticleEngine implements PreparableReloadListener {
   ...
   private static boolean loggedNullState;
   ...
   public CompletableFuture<Void> reload(
      ...
      // Parameterised against the decompiler's raw types; the generics are what vanilla
      // declares and are needed for the stream and method-reference calls below.
      ConcurrentMap<ResourceLocation, List<ResourceLocation>> var7 = Maps.newConcurrentMap();
   ...
   private void loadParticleDescription(ResourceManager var1, ResourceLocation var2, Map<ResourceLocation, List<ResourceLocation>> var3) {
      ...
      try {
         try (
            ...
            ParticleDescription var7 = ParticleDescription.fromJson(GsonHelper.parse(var6));
            List<ResourceLocation> var8 = var7.getTextures();
            ...
            if (var8 == null) {
               ...
               var3.put(var2, var8.stream().map(var0 -> new ResourceLocation(var0.getNamespace(), "particle/" + var0.getPath())).collect(Collectors.toList()));
            }
         }
   ...
   /**
    * Ticks the live particles.
    *
    * <p>Minecraft calls this every tick whenever the game is not paused - see Minecraft.tick,
    * where it sits outside the {@code level != null} check that guards animateTick just above
    * it. Vanilla gets away with that because setLevel(null) empties the particle map, so the
    * body below never runs without a level. Two things break that here: the map can hold a null
    * key if a particle ever reports a null render type, and the level reference can be absent
    * while particles remain queued. Either one is an unhandled NullPointerException on the
    * client thread, which ends the process - it killed the client a few seconds after joining,
    * with the world already drawn and the player standing in it.
    *
    * <p>Skipping a tick, or one malformed bucket of particles, costs a frame of particle
    * animation. The log names the case once so it stays visible without flooding: this runs
   ...
   public void tick() {
      if (this.level == null) {
         if (!loggedNullState) {
            ...
            LOGGER.warn("ParticleEngine.tick with no level and {} particle buckets queued; skipping"
               + " (further occurrences not logged)", this.particles.size());
         }
         return;
      }

      this.particles.forEach((var1x, var2) -> {
         // Every dereference in this lambda, guarded. The previous version checked the key and
         // the level and still threw here, which left the profiler as the only candidate: the
         // level's profiler is supplied by a method reference handed to ClientLevel's
         // constructor, and nothing on this path requires it to have produced anything.
         ProfilerFiller var3x = this.level == null ? null : this.level.getProfiler();
         if (var1x == null || var2 == null || var3x == null) {
            if (!loggedNullState) {
               loggedNullState = true;
               LOGGER.warn("ParticleEngine.tick skipping a bucket: renderType={} particles={}"
                  + " profiler={} (further occurrences not logged)",
                  var1x, var2 == null ? "null" : Integer.toString(var2.size()), var3x);
            }
            return;
         }
         var3x.push(var1x.toString());
   ...
   private void tickParticleList(Collection<Particle> var1) {
      if (!var1.isEmpty()) {
         ...
         while (var2.hasNext()) {
            ...
            // tickParticle wraps Particle.tick in its own catch, but the two dereferences after
            // it do not, so a null particle - or a particle returning null where an Optional is
            // declared - is a raw NullPointerException on the client thread and the process
            // ends. TeaVM inlines this method into tick's lambda, which is why the crash showed
            // no frame of its own and guarding the lambda's own values changed nothing.
            //
            // Deliberately plain: no continue, no logging, no string building. Richer versions
            // of this guard made TeaVM's build daemon overflow its stack and emit nothing at
            // all, which is how builds 142, 152, 156 and 160 were lost.
            if (var3 == null) {
               var2.remove();
            } else {
               this.tickParticle(var3);
               if (!var3.isAlive()) {
                  Optional<ParticleGroup> var4 = var3.getParticleGroup();
                  if (var4 != null) {
                     var4.ifPresent(var1x -> this.updateCount(var1x, -1));
                  }
   ...
   public void render(PoseStack var1, MultiBufferSource.BufferSource var2, LightTexture var3, Camera var4, float var5) {
      ...
      for (ParticleRenderType var8 : RENDER_ORDER) {
         Iterable<Particle> var9 = this.particles.get(var8);
         if (var9 != null) {
               ...
               for (int var24 = 0; var24 < var21; var24++) {
                  for (int var25 = 0; var25 < var22; var25++) {
                     for (int var26 = 0; var26 < var23; var26++) {
                        double var27 = (var24 + 0.5) / var21;
                        double var29 = (var25 + 0.5) / var22;
                        double var31 = (var26 + 0.5) / var23;
                        double var33 = var27 * var15 + var3x;
                        double var35 = var29 * var17 + var5;
                        double var37 = var31 * var19 + var7;
                        this.add(
                           new TerrainParticle(
                              this.level, var1.getX() + var33, var1.getY() + var35, var1.getZ() + var37, var27 - 0.5, var29 - 0.5, var31 - 0.5, var2, var1
                           )
                        );
                     }
                  }
   ...
   public void crack(BlockPos var1, Direction var2) {
      ...
      if (var3.getRenderShape() != RenderShape.INVISIBLE) {
         ...
         double var9 = var4 + this.random.nextDouble() * (var8.maxX - var8.minX - 0.2F) + 0.1F + var8.minX;
         double var11 = var5 + this.random.nextDouble() * (var8.maxY - var8.minY - 0.2F) + 0.1F + var8.minY;
         double var13 = var6 + this.random.nextDouble() * (var8.maxZ - var8.minZ - 0.2F) + 0.1F + var8.minZ;
         if (var2 == Direction.DOWN) {
            var11 = var5 + var8.minY - 0.1F;
         }

         if (var2 == Direction.UP) {
            var11 = var5 + var8.maxY + 0.1F;
         }

         if (var2 == Direction.NORTH) {
            var13 = var6 + var8.minZ - 0.1F;
         }

         if (var2 == Direction.SOUTH) {
            var13 = var6 + var8.maxZ + 0.1F;
         }

         if (var2 == Direction.WEST) {
            var9 = var4 + var8.minX - 0.1F;
         }

         if (var2 == Direction.EAST) {
            var9 = var4 + var8.maxX + 0.1F;
         }
