...
public class ConnectScreen extends Screen {
   ...
   /**
    * Hands multiplayer joins to EaglercraftX's connector instead of opening a socket.
    *
    * <p>Vanilla's {@code connect} below resolves the address and calls
    * {@code Connection.connectToServer}, which in this build is a stub that throws
    * {@code UnsupportedOperationException("EaglercraftX opens its own WebSocket")} - there is no
    * TCP in a page. So every join died there, one step after the address check.
    *
    * <p>{@code GuiConnecting} is EaglercraftX's replacement and was already ported to 1.18.2 -
    * it opens the WebSocket, runs {@code ConnectionHandshake}, builds a
    * {@code WebSocketNetworkManager} and installs a {@code ClientPacketListener}. Nothing called
    * it, exactly as nothing called {@code launchEaglercraftServer} for singleplayer.
    *
    * <p>Redirecting here rather than at each call site covers both of them at once:
    * {@code JoinMultiplayerScreen.join} for the server list and direct connect, and
    * {@code Minecraft}'s {@code --server} quick-play argument.
    *
    * <p>The three calls vanilla makes before showing its screen are kept. They are not
    * bookkeeping: {@code clearLevel} tears down any world still loaded, and
    * {@code prepareForMultiplayer} resets the game mode and player list, both of which matter
    * when joining a server straight from a singleplayer world.
    */
   public static void startConnecting(Screen var0, Minecraft var1, ServerAddress var2, @Nullable ServerData var3) {
      ...
      if (var3 != null) {
         var1.setScreen(new net.minecraft.client.multiplayer.GuiConnecting(var0, var1, var3));
      } else {
         var1.setScreen(new net.minecraft.client.multiplayer.GuiConnecting(
            var0, var1, var2.getHost(), var2.getPort()));
      }
   }
      ...
      Thread var3 = new Thread("Server Connector #" + UNIQUE_THREAD_ID.incrementAndGet()) {
         ...
         public void run() {
            ...
            try {
               ...
               Exception var3x;
               if (var6.getCause() instanceof Exception var4) {
                  var3x = var4;
               } else {
                  var3x = var6;
               }
               ...
               var1.execute(
                  () -> var1.setScreen(
                     new DisconnectedScreen(
                        ConnectScreen.this.parent, CommonComponents.CONNECT_FAILED, new TranslatableComponent("disconnect.genericReason", var7)
                     )
                  )
               );
            }
