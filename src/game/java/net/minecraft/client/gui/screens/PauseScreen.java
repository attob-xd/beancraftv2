...
/**
 * Copied so "leave world" knows what a singleplayer world is in this build.
 *
 * <p>vanilla branches on {@code Minecraft.isLocalServer()}, which is "is there an
 * IntegratedServer object in this process". EaglercraftX's server is in a Web Worker, so that
 * is false even while you are playing singleplayer - and every singleplayer session therefore
 * took the remote-server path: the button read "Disconnect", the world was cleared without the
 * "Saving world" screen, and leaving dropped you on the multiplayer server list instead of the
 * title screen. {@link #isSingleplayer()} asks the question this build can actually answer.
 */
public class PauseScreen extends Screen {

   /** True while EaglercraftX's integrated server is running this world. */
   private boolean isSingleplayer() {
      return net.lax1dude.eaglercraft.sp.SingleplayerServerController.isWorldRunning();
   }
   ...
   private void createPauseMenu() {
      ...
      TranslatableComponent var5 = this.isSingleplayer()
      ...
      this.addRenderableWidget(new Button(this.width / 2 - 102, this.height / 4 + 120 + -16, 204, 20, var5, var1x -> {
         boolean var2x = this.isSingleplayer();
