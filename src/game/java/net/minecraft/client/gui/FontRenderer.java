...
public class FontRenderer extends Font implements ResourceManagerReloadListener {
	...
	public FontRenderer(Options gameSettingsIn, ResourceLocation location, TextureManager textureManagerIn,
			boolean unicode) {
		super(var1 -> null);
		this.locationFontTexture = location;
		this.renderEngine = textureManagerIn;
		this.unicodeFlag = unicode;
		textureManagerIn.bindForSetup(this.locationFontTexture);
		...
		for (int i = 0; i < 32; ++i) {
			...
			if (false) { // anaglyph 3D: removed after 1.12.2, colours are never remapped now
	...
	private void readFontTexture() {
		...
		try {
			...
			bufferedimage = ImageData.loadImageFile(iresource.getInputStream());
	...
	private float renderDefaultChar(int ch, boolean italic) {
		...
		int j = ch / 16 * 8;
		int k = italic ? 1 : 0;
		this.renderEngine.bindForSetup(this.locationFontTexture);
	...
	private void loadGlyphTexture(int page) {
		this.renderEngine.bindForSetup(this.getUnicodePageLocation(page));
	}
		...
		for (int i = 0; i < text.length(); ++i) {
			...
			if (c0 == 167 && i + 1 < text.length()) {
				...
				if (this.strikethroughStyle) {
					...
					bufferbuilder.pos((double) this.posX, (double) (this.posY + (float) (this.lineHeight / 2)), 0.0D)
							.endVertex();
					bufferbuilder
							.pos((double) (this.posX + f), (double) (this.posY + (float) (this.lineHeight / 2)), 0.0D)
							.endVertex();
					bufferbuilder.pos((double) (this.posX + f),
							(double) (this.posY + (float) (this.lineHeight / 2) - 1.0F), 0.0D).endVertex();
					bufferbuilder
							.pos((double) this.posX, (double) (this.posY + (float) (this.lineHeight / 2) - 1.0F), 0.0D)
							.endVertex();
					tessellator.draw();
				...
				if (this.underlineStyle) {
					...
					bufferbuilder1.pos((double) (this.posX + (float) l),
							(double) (this.posY + (float) this.lineHeight), 0.0D).endVertex();
					bufferbuilder1.pos((double) (this.posX + f), (double) (this.posY + (float) this.lineHeight), 0.0D)
							.endVertex();
					bufferbuilder1
							.pos((double) (this.posX + f), (double) (this.posY + (float) this.lineHeight - 1.0F), 0.0D)
							.endVertex();
					bufferbuilder1.pos((double) (this.posX + (float) l),
							(double) (this.posY + (float) this.lineHeight - 1.0F), 0.0D).endVertex();
					tessellator1.draw();
					GlStateManager.enableTexture2D();
				}
	...
	private void renderSplitString(String str, int x, int y, int wrapWidth, boolean addShadow) {
		List<String> lst = EaglerFontCompat.listFormattedStringToWidth(this, str, wrapWidth);
	...
	public int splitStringWidth(String str, int maxLength) {
		return this.lineHeight * EaglerFontCompat.listFormattedStringToWidth(this, str, maxLength).size();
	}
	...
	// ---- 1.18.2 Font API, bridged onto the 1.12.2 implementations above ----
	// 1.18.2 threads a PoseStack through drawing; this renderer draws immediately with
	// its own GL state, so the stack is not consulted.
