...
/**
 * Vanilla's class, with the three projection builders emitting a <b>reversed-Z</b> matrix.
 *
 * <p>EaglercraftX renders reversed: near maps to depth 1, far to 0, the depth buffer clears
 * to 0, and every comparison is flipped - {@code GlStateManager.depthFunc} literally rewrites
 * LEQUAL to GEQUAL and LESS to GREATER on the way to the driver. Its own matrix builders are
 * written to match; compare {@code GlStateManager.ortho}, which uses {@code m22 = 2/(zFar-zNear)}
 * where textbook GL ortho uses {@code -2/(zFar-zNear)}, and {@code gluPerspective}, flipped the
 * same way.
 *
 * <p>1.12.2 went through those builders for every projection it set. 1.18.2 does not: it
 * composes its own matrices here and hands them to a shader uniform, so the reversal was
 * silently dropped while the flipped comparison stayed. The two then disagreed, and z ordering
 * inverted - anything drawn <i>in front</i> of what was already in the depth buffer was
 * rejected.
 *
 * <p>On screens built around a selection list that was stark: {@code AbstractSelectionList}
 * fills its own rectangle with {@code depthFunc(GL_ALWAYS)}, so only that rectangle ever got a
 * depth value, and {@code AbstractWidget.renderButton} turns depth testing back on for every
 * button. The list and its entries drew; the title above it and the whole button row below it
 * were discarded. The buttons were still there and still took clicks - they just never
 * produced a pixel, which is exactly how it looked: "where are the buttons".
 *
 * <p>Negating m22 and m23 mirrors z about the middle of the clip range, which is precisely the
 * difference between the two conventions, and leaves x, y and w untouched. {@code perspective}
 * keeps {@code m32 = -1} - that is the perspective divide, not a depth term.
 */
public final class Matrix4f {
   ...
   private static boolean isInteger(float var0) {
      return Math.abs(var0 - Math.round(var0)) <= 1.0E-5;
   }
   ...
   public static Matrix4f perspective(double var0, float var2, float var3, float var4) {
      float var5 = (float)(1.0 / Math.tan(var0 * (float) (Math.PI / 180.0) / 2.0));
      Matrix4f var6 = new Matrix4f();
      var6.m00 = var5 / var2;
      var6.m11 = var5;
      // Reversed: negated against vanilla. See the class comment.
   ...
   public void translate(Vector3f var1) {
      this.m03 = this.m03 + var1.x();
      this.m13 = this.m13 + var1.y();
      this.m23 = this.m23 + var1.z();
   }

   /**
    * A copy with the reversed-Z convention undone, for code that does its own projection maths.
    *
    * <p>The builders above emit reversed-Z to match the rest of the engine - see the class
    * header. That is right for anything handed to the driver, because the depth comparison is
    * reversed to match. It is wrong for anything that re-derives geometry from the matrix on the
    * CPU using textbook formulas, because those assume near maps low and far maps high.
    *
    * <p>Frustum is the case that matters: it extracts six clipping planes and a view vector from
    * projection x modelview. Fed a reversed matrix, its near and far planes come out inverted,
    * so a box in front of the camera fails the test and everything is culled - no chunk is ever
    * compiled, and the client sits on "Loading terrain" forever with a perfectly good world
    * behind it.
    */
   public Matrix4f unreversedZCopy() {
   ...
   public void multiplyWithTranslation(float var1, float var2, float var3) {
      this.m03 = this.m00 * var1 + this.m01 * var2 + this.m02 * var3 + this.m03;
      this.m13 = this.m10 * var1 + this.m11 * var2 + this.m12 * var3 + this.m13;
      this.m23 = this.m20 * var1 + this.m21 * var2 + this.m22 * var3 + this.m23;
      this.m33 = this.m30 * var1 + this.m31 * var2 + this.m32 * var3 + this.m33;
   }
