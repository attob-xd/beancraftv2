...
public class Uniform extends AbstractUniform implements AutoCloseable {
   ...
   private static boolean loggedNullSet;
   ...
   public final void set(Vector3f var1) {
      // Neither of these is checked upstream. RenderSystem.setupShaderLights runs on every
      // buffer upload and passes shaderLightDirections[0] straight in, guarding the uniform but
      // not the vector, and floatValues is null on a uniform whose declared type is an integer
      // one. Either way the dereference lands here, several frames of renderer call stack from
      // whatever is actually wrong - it showed up as the player's hand killing the client after
      // the entire world had drawn correctly.
      //
      // Skipping the write leaves the light direction at its previous value, which is wrong in
      // a way nobody can see, rather than ending the frame and the process. The log says which
      // half was null and names the uniform, once per occurrence.
      if (var1 == null || this.floatValues == null) {
         // Once, not once per uniform per frame: this fires from a buffer upload, so left
         // unthrottled it produced hundreds of lines a second and pushed every other message
         // out of the console ring buffer - including the one that explained the null.
         if (!loggedNullSet) {
            loggedNullSet = true;
            LOGGER.error("Uniform {} set(Vector3f) with vector={} floatValues={}; skipping"
               + " (further occurrences not logged)", this.name, var1, this.floatValues);
         }
         return;
      }
