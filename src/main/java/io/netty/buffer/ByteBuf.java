...
public abstract class ByteBuf implements Comparable<ByteBuf> {
	...
	/*
	 * Reference counting. Real netty pools its buffers and frees them by refcount; the
	 * vendored ByteBuf here is backed by a plain array that the garbage collector owns, so
	 * there is nothing to free and these are bookkeeping only. 1.12.2 never called them -
	 * 1.18.2's FriendlyByteBuf does, and a missing release() is a link error, not a leak.
	 */
