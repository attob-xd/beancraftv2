...
public class GuiCredits extends GuiScreenCompat {
	...
	public GuiCredits(Screen screen, String location) {
			...
			for (String s : lines) {
				...
				if (s2.isEmpty()) {
					...
					for (String s3 : words) {
						...
						if (this.minecraft.font.width(cCurrentLine) < width) {
	...
	protected void actionPerformed(GuiButtonCompat button) throws IOException {
	}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		int x = (width - 345) / 2;
		int y = (height - 230) / 2;
		EaglerGuiCompat.bindGuiTexture(creditsBck);
		EaglerGuiCompat.currentPoseStack().pushPose();
		EaglerGuiCompat.currentPoseStack().translate(x, y, 0.0f);
		EaglerGuiCompat.currentPoseStack().scale(1.39f, 1.39f, 1.39f);
		this.drawTexturedModalRect(0, 0, 0, 0, 248, 166);
		EaglerGuiCompat.currentPoseStack().popPose();
		EaglerGuiCompat.bindGuiTexture(beaconx);
		...
		for (int i = 0; i < visibleLines; ++i) {
			this.minecraft.font.draw(net.lax1dude.eaglercraft.compat.EaglerGuiCompat.currentPoseStack(), this.credits.get(scrollPosition + i), x + 10, y + 10 + (i * 10),
					0x404060);
		}
		int trackHeight = 193;
