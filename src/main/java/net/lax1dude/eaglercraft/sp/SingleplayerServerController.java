...
public class SingleplayerServerController {
	...
	public static void launchEaglercraftServer(String mcDataDir, String folderName, String worldName, int viewDistance,
			EaglerWorldSettings settings) {
		ensureReady();
		clearTPS();
		int difficulty = Minecraft.getInstance().options.difficulty.getId();

		if (settings != null) {
			sendIPCPacket(new IPCPacket02InitWorld(folderName, settings.getGameType().getId(),
					settings.getTerrainType().hashCode(), settings.getGeneratorOptions(), settings.getSeed(),
	...
	private static void handleIPCPacket(IPCPacketBase ipc) {
		...
		case IPCPacketFFProcessKeepAlive.ID: {
			...
			case 0xFF:
				...
				sendIPCPacket(new IPCPacket14StringList(IPCPacket14StringList.LOCALE, new String[0]));
...
//			}
	...
	public static boolean isClientInEaglerSingleplayer() {
		...
		return mc != null && mc.player != null && net.lax1dude.eaglercraft.compat.EaglerClientState.get(mc.player.connection).isClientInEaglerSingleplayer();
	}
