...
/*
 * PATCH (EaglercraftX 1.18.2 port), on top of TeaVM's own GraphUtils:
 *
 * findBackEdges, removeLoops and dfs each ran their depth-first search on a fixed
 * `new int[size * 2]` stack. The search pushes a node once for itself and once per
 * edge into a not-yet-visited successor, so any control-flow graph with more edges
 * than nodes can push past 2n and throw
 *
 *     ArrayIndexOutOfBoundsException: Index 42 out of bounds for length 42
 *
 * out of AsyncProgramSplitter, killing the build with no method name attached.
 * 1.12.2 never hit it; 1.18.2's async-split graphs are denser. The stacks now grow.
 *
 * Compiled against teavm-core-0.9.2 into src/teavmc-classpath/resources, which
 * precedes the TeaVM jars on the buildscript classpath - the same mechanism lax1dude
 * already uses there to replace long.js. Rebuild with tools/build_teavm_patches.sh.
 */
public final class GraphUtils {
    ...
    /** Pushes onto a DFS stack that grows instead of overflowing. */
    private static int[] push(int[] stack, int size, int value) {
    ...
    /**
     * The depth-first searches below push a node once for itself and once for every edge
     * that reaches it, so 2n is not a safe bound - see the header comment. This is: every
     * node, plus every edge, plus one.
     */
    private static int dfsStackSize(Graph graph) {
        int size = graph.size();
        int total = size + 1;
        for (int i = 0; i < size; ++i) {
            total += graph.incomingEdgesCount(i);
        }
        return total;
    }
    ...
    public static int[] findBackEdges(Graph graph) {
        int sz = graph.size();
        int[] stack = new int[dfsStackSize(graph)];
        int stackSize = 0;
